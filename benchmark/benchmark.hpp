...
eytzinger_maker(RandomIt1 begin, RandomIt1 end, RandomIt2 begin_prev_data, size_t i = 0, size_t k = 1) {
  if (k <= end - begin) {
    i = eytzinger_maker<RandomIt1, RandomIt2, K>(begin, end, begin_prev_data, i, 2 * k);
    begin[k - 1] = K(begin_prev_data[i++]);
    i = eytzinger_maker<RandomIt1, RandomIt2, K>(begin, end, begin_prev_data, i, 2 * k + 1);
  }
  return i;
}
...
eytzinger(RandomIt begin, RandomIt end, typename RandomIt::value_type q) {
  ...
  size_t n = end - begin;
  while (k <= n) {
    __builtin_prefetch(static_cast<void *>(begin.get() + k * 16));
    k = 2 * k + (begin[k - 1] < q);
  }
  k >>= __builtin_ffs(~k);
  return begin + (k - 1);
